...
- Unitaire : `extraction-illegible` et `extraction-failed` produisent bien
  deux poses distinctes (`sorry` vs `glitch`) ; `extraction-illegible` et
  `extraction-not-a-course-page` partagent la pose mais jamais la phrase — le test qui protège la
